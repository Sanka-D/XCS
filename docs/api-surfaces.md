...
`NUXT_DATABASE_URL` is the read-only projection connection (`xcs_api`). Optional publication
and demo pinning require a separate `NUXT_PAYLOAD_DATABASE_URL` (`xcs_payload_writer`), whose
