...
For local Compose with an HTTPS gateway, add the optional auth overlay **after** the secret overlay:

```sh
docker compose -f docker-compose.yml -f docker-compose.secrets.yml -f docker-compose.auth.yml config --quiet
docker compose -f docker-compose.yml -f docker-compose.secrets.yml -f docker-compose.auth.yml run --rm db-bootstrap
docker compose -f docker-compose.yml -f docker-compose.secrets.yml -f docker-compose.auth.yml up -d web
```

Set the additional host file paths `XCS_APP_DATABASE_PASSWORD_FILE`, `NUXT_APP_DATABASE_URL_FILE`,
`XCS_IDENTITY_CLIENT_ID_FILE` and `XCS_IDENTITY_CLIENT_SECRET_FILE`. The application URL uses the
same application password, e.g. the Compose database hostname `postgres`. Bootstrap receives the
password; only Nuxt receives the URL and OIDC client files. Keep the parent directory private and
make individual mounted files readable by uid 1000. No application secret is mounted into indexer.
...
```sh
pnpm --filter @xcs-protocol/web exec vitest run test/auth-config.test.ts test/auth-oidc.test.ts test/auth-http.test.ts test/walletLinkProof.test.ts
# Set XCS_TEST_DATABASE_URL to a disposable PostgreSQL 18 cluster first.
pnpm test:postgres
pnpm --filter @xcs-protocol/web test:e2e:auth
```
