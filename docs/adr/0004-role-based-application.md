# ADR 0004: role-based application boundary

Status: accepted for portal admission and private-claim access; not yet exposed by the portal.
...
[ADR 0002](0002-public-product-and-discovery.md) describes the accountless Testnet beta.
The next application phase introduces accounts and operator-approved roles on the Commons portal.
This record distinguishes the accepted product direction from implemented behavior: the current
application is still accountless. No authorization or database change is introduced by this ADR.
...
publishing schemas or issuing credentials through the future role-based Commons portal. Verifiers
...
Use these accepted access decisions when designing the application data model and authorization
contracts. Implement server-side role enforcement together with the corresponding application
flows, not by hiding buttons alone. Update the current-behavior documentation and define the
accountless Studio transition when those changes are implemented; do not advertise approval gates
as live before then. ADR 0002 remains the current beta baseline, with this record defining the
accepted portal-access direction for its successor.
