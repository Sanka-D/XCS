# ADR 0005: serve the API in Nuxt with separate database privileges
...
- `NUXT_DATABASE_URL` uses `xcs_api` for read-only ledger projections.
- Optional `NUXT_PAYLOAD_DATABASE_URL` uses `xcs_payload_writer` for hosted payloads and demo
...
introducing an `/api/v1` prefix. Domain services live under `server/xcs/api` and remain independent
