...
import { createApi } from '../helpers/http.js'
import type { OperationalMetricsRepository } from '../../../server/xcs/api/operational-metrics.js'
import {
  MAX_SCHEMA_CATALOG_ENTRIES,
  type SchemaCatalogBundle,
} from '../../../server/xcs/api/schema-catalog.js'
import { canonicalJson, encodeUtf8, sha256Hex } from '../../../server/xcs/api/serialization.js'
import type { ApiRepository, SchemaProjectionEvidence } from '../../../server/xcs/api/types.js'
import { StaticTrustPolicy } from '../../../server/xcs/api/verification.js'
...
describe('read API', () => {
  it('serves native HEAD responses and a downloadable OpenAPI contract', async () => {
    const instance = await app()
    const head = await instance.inject({ method: 'HEAD', url: '/health/live' })
    expect(head.statusCode).toBe(200)
    expect(head.body).toBe('')
    expect(head.headers['cache-control']).toBe('no-store')
    const document = await instance.inject({ method: 'GET', url: '/documentation/json' })
    expect(document.statusCode).toBe(200)
    expect(document.json()).toMatchObject({ openapi: '3.0.3' })
    expect(document.json().paths).toHaveProperty('/v1/verify')
    const documentation = await instance.inject({ method: 'GET', url: '/documentation' })
    expect(documentation.statusCode).toBe(200)
    expect(documentation.body).toContain('/documentation/json')
  })

  it('bounds chunked request bodies without relying on Content-Length', async () => {
    const instance = await app()
    const response = await instance.inject({
      method: 'POST',
      url: '/v1/verify',
      headers: { 'content-type': 'application/json', 'transfer-encoding': 'chunked' },
      payload: Readable.from(Array.from({ length: 19 }, () => Buffer.alloc(64 * 1024, 32))),
    })
    expect(response.statusCode).toBe(413)
    expect(response.json()).toMatchObject({ error: 'REQUEST_ERROR' })
  })

  it('reads bounded JSON bodies through the native H3 Web adapter', async () => {
    const instance = await app()
    ...
    const response = await request(
      JSON.stringify({ network: 'testnet', issuer: ISSUER, subject: SUBJECT, schemaUid: UID }),
    )
    expect(response.status).toBe(200)
    expect(await response.json()).toHaveProperty('onChain')
    const oversized = await request(' '.repeat(2 * 1024 * 1024))
  ...
  it('rejects weak operational metrics tokens at construction', async () => {
    await expect(
      configuredApp({
        operationalMetrics: {
          token: 'too-short',
  ...
  it('does not trust legacy SSR headers or forwarded addresses without a trusted proxy', async () => {
    const instance = await configuredApp({ globalRateLimit: 1 })
    const request = (address: string, key: string) =>
      instance.inject({
        method: 'GET',
        url: '/v1/networks',
        headers: {
          'x-forwarded-for': address,
          'x-xcs-internal-token': 'test-internal-ssr-token-000000000001',
          'x-xcs-client-key': key,
        },
      })
    expect((await request('198.51.100.10', 'a'.repeat(64))).statusCode).toBe(200)
    expect((await request('198.51.100.11', 'b'.repeat(64))).statusCode).toBe(429)
  })
