...
import { computeSchemaUid, encodeCredentialPayload, payloadDigest } from '@xcs-protocol/core'
import { createDatabaseClient, type DatabaseClient } from '@xcs-protocol/db'
...
describe.skipIf(!enabled)('built Nuxt with real PostgreSQL', () => {
  it('keeps independent SSR quota identities through the real Nitro localFetch dispatcher', async () => {
    await access(outputEntry)
    const port = await unusedPort()
    const script = `
      await import(${JSON.stringify(new URL('../../.output/server/index.mjs', import.meta.url).href)})
      const request = (address) => globalThis.$fetch.raw('/v1/verify', {
        method: 'POST', body: {}, retry: 0, timeout: 2000, ignoreResponseError: true,
        context: { xcsClientAddress: address },
      })
      const statuses = []
      ...
      const other = await request('198.51.100.11')
      process.send({ statuses, other: other.status })
    `
    const server = spawn(process.execPath, ['--input-type=module', '--eval', script], {
      ...
      env: {
        ...
        NUXT_DATABASE_URL: 'postgres://xcs_api:unused@127.0.0.1:1/xcs_unused',
      },
    })
    try {
      ...
      expect(result).toMatchObject({ statuses: expect.arrayContaining([400, 429]), other: 400 })
    } finally {
    ...
    try {
      ...
      server = spawn(process.execPath, [outputEntry], {
        ...
        env: {
          ...
          NITRO_HOST: '127.0.0.1',
          NITRO_PORT: String(port),
          NUXT_DATABASE_URL: roleUrl(parsed.toString(), 'xcs_api', fixturePasswords.apiPassword),
          NUXT_PAYLOAD_DATABASE_URL: roleUrl(
            parsed.toString(),
            'xcs_payload_writer',
