...
describe('client quota identity', () => {
  it('preserves only server-owned client context through Nitro localFetch', async () => {
    for (const address of ['198.51.100.10', '198.51.100.11']) {
      ...
      await clientAddressMiddleware(event)
      expect(event.context.xcsClientAddress).toBe(address)
    }
  })
  ...
  it('does not accept client-context headers as an internal SSR identity', async () => {
    ...
    const event = createEvent(request, new ServerResponse(request))
    await clientAddressMiddleware(event)
    expect(event.context.xcsClientAddress).toBe('unresolved-peer')
  })
