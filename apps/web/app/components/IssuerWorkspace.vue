...
async function action(work: () => Promise<unknown>) {
  ...
  try {
    await work()
    notice.value = t('issuer.recorded')
    await refresh()
  } catch {
    failure.value = true
  } finally {
...
async function invite() {
  ...
  await action(async () => {
    await mutate('/api/issuer/invites', {
...
useSeoMeta({ title: () => `${t(`issuer.${props.section}`)} — XCS`, robots: 'noindex,nofollow' })
</script>

<template>
  <PageHeader :title="$t(`issuer.${section}`)" :lead="$t('issuer.intro')" />
  ...
  <StatusBox v-if="notice" tone="success" role="status" class="mb-5">{{ notice }}</StatusBox>
  ...
  <template v-if="data">
    ...
    <template v-else-if="section === 'schemas'">
      <UButton
        ...
        >{{ $t('issuer.publishSchema') }}</UButton
      >
      <p v-if="!data.schemas.length" class="mt-6 text-muted">{{ $t('issuer.noSchemas') }}</p>
      <ul class="mt-6 grid gap-4 sm:grid-cols-2">
        <li v-for="schema in data.schemas" :key="`${schema.profileId}:${schema.schemaUid}`">
          <UCard>
            ...
            <UButton
              class="mt-3"
              :to="{ path: localePath('/issuer/recipients'), query: { organizationId: selected } }"
              >{{ $t('issuer.invite') }}</UButton
            >
          </UCard>
        </li>
      </ul>
    ...
    <template v-else-if="section === 'recipients'">
      <UCard>
        <h2 class="text-xl font-semibold">{{ $t('issuer.invite') }}</h2>
        <p class="mt-2 text-muted">{{ $t('issuer.inviteHelp') }}</p>
        <form class="mt-4 grid gap-4" @submit.prevent="invite">
          ...
          <label
            >{{ $t('issuer.schemas')
              ...
              class="mt-1 block w-full rounded border border-default bg-default p-3"
            >
              <option value="" disabled>{{ $t('issuer.chooseSchema') }}</option>
              <option
      ...
      <ul class="mt-6 grid gap-4">
        <li v-for="item in data.invites" :key="item.id">
          <UCard>
            ...
            <p v-if="item.deliveryStatus === 'uncertain'" class="mt-2 text-sm">
              {{ $t('issuer.uncertain') }}
            </p>
      ...
      <ul class="grid gap-4">
        <li v-for="item in data.credentials" :key="item.generationId">
          <UCard>
            ...
            <p>{{ $t(`issuer.visibility.${item.visibility}`) }}</p>
