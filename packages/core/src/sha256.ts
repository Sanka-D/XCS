import { bytesToHex } from './utf8.js'
...
/** Dependency-free SHA-256 so protocol computations stay synchronous in browsers. */
export function sha256(bytes: Uint8Array): Uint8Array {
  ...
  for (let chunk = 0; chunk < paddedLength; chunk += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 = (h + upper1 + choice + (ROUND[index] ?? 0) + (words[index] ?? 0)) >>> 0
